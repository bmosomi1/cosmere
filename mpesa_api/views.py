...
from mpesa_api.models import Mpesa, GathuMpesa, ExperientialMpesa, ExperientialMpesaArchive
from mpesa_api.utils import *
...
@csrf_exempt
def gathu_register_urls(request):
    access_token = mpesa_access_token(Gathu.CONSUMER_KEY, Gathu.CONSUMER_PASSWORD)
    api_url = "https://api.safaricom.co.ke/mpesa/c2b/v1/registerurl"
    headers = {"Authorization": "Bearer %s" % access_token}
    options = {"ShortCode": "4070687",
               "ResponseType": "Completed",
               "ConfirmationURL": "https://roberms.co.ke/api/v1/c2b/gathu/4070687/confirmation",
               "ValidationURL": "https://roberms.co.ke/api/v1/c2b/gathu/4070687/validation"}
    response = requests.post(api_url, json=options, headers=headers)
    return HttpResponse(response.text)


@csrf_exempt
def gathu_validation(request):
    context = {
        "ResultCode": 0,
        "ResultDesc": "Completed"
    }
...
@csrf_exempt
def gathu_confirmation(request):
    mpesa_body = request.body.decode('utf-8')
    mpesa_payment = json.loads(mpesa_body)
...
@csrf_exempt
def chrisbe_register_urls(request):
    access_token = mpesa_access_token(Chrisbe.CONSUMER_KEY, Chrisbe.CONSUMER_PASSWORD)
    api_url = "https://api.safaricom.co.ke/mpesa/c2b/v1/registerurl"
    headers = {"Authorization": "Bearer %s" % access_token}
    options = {"ShortCode": "4070685",
               "ResponseType": "Completed",
               "ConfirmationURL": "https://roberms.co.ke/api/v1/c2b/chrisbe/4070685/confirmation",
               "ValidationURL": "https://roberms.co.ke/api/v1/c2b/chrisbe/4070685/validation"}
    response = requests.post(api_url, json=options, headers=headers)
    return HttpResponse(response.text)


@csrf_exempt
def chrisbe_validation(request):
    context = {
        "ResultCode": 0,
        "ResultDesc": "Completed"
    }
...
@csrf_exempt
def chrisbe_confirmation(request):
    mpesa_body = request.body.decode('utf-8')
    mpesa_payment = json.loads(mpesa_body)
    GathuMpesa.objects.create(
        first_name=mpesa_payment['FirstName'],
...
@csrf_exempt
def experiential_register_urls(request):
    access_token = mpesa_access_token(Experiential.CONSUMER_KEY, Experiential.CONSUMER_PASSWORD)
    api_url = "https://api.safaricom.co.ke/mpesa/c2b/v1/registerurl"
    headers = {"Authorization": "Bearer %s" % access_token}
    options = {"ShortCode": "4048443",
               "ResponseType": "Completed",
               "ConfirmationURL": "https://roberms.co.ke/api/v1/c2b/experiential/4048443/confirmation",
               "ValidationURL": "https://roberms.co.ke/api/v1/c2b/experiential/4048443/validation"}
    response = requests.post(api_url, json=options, headers=headers)
    return HttpResponse(response.text)


@csrf_exempt
def experiential_validation(request):
    context = {
        "ResultCode": 0,
        "ResultDesc": "Completed"
    }
...
@csrf_exempt
def experiential_confirmation(request):
    mpesa_body = request.body.decode('utf-8')
    mpesa_payment = json.loads(mpesa_body)
    ExperientialMpesa.objects.create(
        first_name=mpesa_payment['FirstName'],
        last_name=mpesa_payment['MiddleName'],
        description=mpesa_payment['TransID'],
        phone_number=mpesa_payment['MSISDN'],
...
@csrf_exempt
def clemode_register_urls(request):
    access_token = mpesa_access_token(Clemode.CONSUMER_KEY, Clemode.CONSUMER_PASSWORD)
    api_url = "https://api.safaricom.co.ke/mpesa/c2b/v1/registerurl"
    headers = {"Authorization": "Bearer %s" % access_token}
    options = {"ShortCode": "4073307",
               "ResponseType": "Completed",
               "ConfirmationURL": "https://roberms.co.ke/api/v1/c2b/clemode/4073307/confirmation",
               "ValidationURL": "https://roberms.co.ke/api/v1/c2b/clemode/4073307/validation"}
    response = requests.post(api_url, json=options, headers=headers)
    return HttpResponse(response.text)


@csrf_exempt
def clemode_validation(request):
    context = {
        "ResultCode": 0,
        "ResultDesc": "Completed"
    }
...
@csrf_exempt
def clemode_confirmation(request):
    mpesa_body = request.body.decode('utf-8')
    mpesa_payment = json.loads(mpesa_body)
    ExperientialMpesa.objects.create(
        first_name=mpesa_payment['FirstName'],
...
@csrf_exempt
def dimples_register_urls(request):
    access_token = mpesa_access_token(Dimples.CONSUMER_KEY, Dimples.CONSUMER_PASSWORD)
    api_url = "https://api.safaricom.co.ke/mpesa/c2b/v1/registerurl"
    headers = {"Authorization": "Bearer %s" % access_token}
    options = {"ShortCode": "4075063",
               "ResponseType": "Completed",
               "ConfirmationURL": "https://roberms.co.ke/api/v1/c2b/dimples/4075063/confirmation",
               "ValidationURL": "https://roberms.co.ke/api/v1/c2b/dimples/4075063/validation"}
    response = requests.post(api_url, json=options, headers=headers)
    return HttpResponse(response.text)


@csrf_exempt
def dimples_validation(request):
    context = {
        "ResultCode": 0,
        "ResultDesc": "Completed"
    }
...
@csrf_exempt
def dimples_confirmation(request):
    mpesa_body = request.body.decode('utf-8')
    mpesa_payment = json.loads(mpesa_body)
    GathuMpesa.objects.create(
        first_name=mpesa_payment['FirstName'],
...
def archive(request):
    ...
    ExperientialMpesaArchive.objects.bulk_create(payments)
    payments.delete()
    return JsonResponse("Done archiving")