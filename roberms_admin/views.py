...
@admin_required
def remove_sales_person_client(request, client_id):
    sale = Sale.objects.filter(customer_id=client_id).first()
    sales_person_id = sale.sales_person_id
    if sale is not None:
        sale.delete()
        return redirect('roberms_admin:sales_person_clients', sales_person_id)
    else:
        return redirect('roberms_admin:dashboard')
