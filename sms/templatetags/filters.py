...
@register.filter(name='get_account_email')
def get_account_email(user_id):
    customer = Customer.objects.filter(user_ptr_id=user_id).first()
    if customer is not None:
        return customer.username
    elif CustomerSubAccounts.objects.filter(user_ptr_id=user_id).first() is not None:
        return CustomerSubAccounts.objects.filter(user_ptr_id=user_id).first().email