...
from integrations.credentials import Roberms, mpesa_access_token
from mpesa_api.models import Mpesa, ExperientialMpesa
...
import logging
from .tasks import simple_sms_store, from_group_send, new_store_contact_task, store_contact_task
...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            if Customer.objects.filter(username=request.POST['email']).count() < 1:
                ...
                OutgoingNew.objects.create(
                    ...
                    text_message="Account created successfully!\nVerify the account with this code "
                                 f"{customer.customer_code} now.\n" +
        ...
        return render(request, 'r/register.html')
...
def reports(request, tracking_code):
    sent_messages = OutgoingDone.objects.filter(
        delivery_status__icontains='DeliveredTo', track_code=tracking_code).count()
    messages_not_sent = OutgoingDone.objects.filter(
        Q(delivery_status__icontains="InvalidMsisdn") | Q(delivery_status__icontains="DeliveryImpossible")|
        Q(delivery_status__icontains="AbsentSubscriber") | Q(delivery_status__icontains="SenderName Blacklisted"),
    ...
    if customer is not None:
        context = {
            ...
            'airtel_messages': OutgoingDone.objects.filter(
                customer_id=customer.id, track_code=tracking_code, delivery_status__contains='success').count()
        }
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if Customer.objects.filter(user_ptr_id=user.id).count() > 0:
                if user.id == 457:
                    return redirect('sms:experiential_payments')
                return redirect('sms:apps')
...
def verify_payment(request):
    if request.method == 'POST':
        user_top_up = Sms_TopUp.objects.filter(
            verifycode=request.POST['verification_code'],
            verified=0
        ).first()

        today = datetime.datetime.now()

        last_three_digits = request.POST['verification_code'][-3:]
        phone_user_top_up = Sms_TopUp.objects.filter(
            user_phone__endswith=last_three_digits,
            verified=0,
            created_at__year=today.year, created_at__day=today.day, created_at__month=today.month
        ).order_by('id').first()
        ...
        if user_top_up:
            ...
            elif float(user_top_up.amount) >= float(500):
                credit = float(user_top_up.amount) / float(0.5)
            # elif 500 <= float(user_top_up.amount) <= float(49999):
            #     credit = float(user_top_up.amount) / float(0.5)
            ...
            if customer is not None:
                ...
                messages.success(request, 'Top Up Successful')
                return redirect('sms:apps')
        elif phone_user_top_up:
            credit = 0
            if float(phone_user_top_up.amount) < 500:
                credit = float(phone_user_top_up.amount) * float(1)
            elif float(phone_user_top_up.amount) >= float(500):
                credit = float(phone_user_top_up.amount) / float(0.5)

            customer = Customer.objects.filter(user_ptr_id=request.user.id).first()
            if customer is not None:
                existing_credit = customer.credit
                customer.credit = existing_credit + credit
                phone_user_top_up.verified = 1
                phone_user_top_up.user_id = customer.id
                phone_user_top_up.created_at = datetime.datetime.now()
                phone_user_top_up.user_phone = f"254{request.POST['verification_code'][-9:]}"
                phone_user_top_up.save()
                customer.save()
...
@login_required()
@is_user_customer
def phone_verify_payment(request):
    if request.method == 'POST':
        first_three_digits = request.POST['verification_code'][:2]
        last_three_digits = request.POST['verification_code'][-3:]
        mpesa_payment = Mpesa.objects.filter(
            phone_number__startswith=first_three_digits,
            phone_number__endswith=last_three_digits,
            processed=False,
            account_number=7690604
        )

        if mpesa_payment.count() > 1:
            messages.error(request, 'More than one payment found, please contact our customer support team.')
            return redirect('sms:verify_payment')
        else:
            mpesa_payment = mpesa_payment.first()

        phone_user_top_up = Sms_TopUp.objects.filter(
            user_phone__startswith=first_three_digits,
            user_phone__endswith=last_three_digits,
            verified=0,
            till_number=7690604
        ).first()
        ...
        if mpesa_payment:
            credit = 0
            if float(mpesa_payment.amount) < 500:
                credit = float(mpesa_payment.amount) * float(1)
            elif float(mpesa_payment.amount) >= float(500):
                credit = float(mpesa_payment.amount) / float(0.5)
            customer = Customer.objects.filter(user_ptr_id=request.user.id).first()
            if customer is not None:
                existing_credit = customer.credit
                customer.credit = existing_credit + credit
                ...
                if phone_user_top_up:
                    phone_user_top_up.verified = True
                    phone_user_top_up.save()

                else:
                    try:
                        Sms_TopUp.objects.create(
                            user_phone=request.POST['verification_code'],
                            transaction_ref=json.loads(mpesa_payment.description.replace("\'", "\""))['TransID'],
                            amount=mpesa_payment.amount,
                            till_number=mpesa_payment.account_number,
                            f_name=mpesa_payment.first_name,
                            l_name=mpesa_payment.last_name,
                            signature=json.loads(mpesa_payment.description.replace("\'", "\""))['TransID'],
                            account_no='TILL NUMBER',
                            transaction_type=mpesa_payment.type,
                            verifycode=request.POST['verification_code'],
                            user_id=customer.id,
                            timestamp=datetime.datetime.now(),
                            verified=1
    ...
    def filter_queryset(self, qs):
        ...
        if filter_customer:
            ...
            for part in customer_parts:
                ...
                qs_params = Union[qs_params, q] if qs_params else q
    ...
    for message in messages:
        try:
            summary_sheet.append(
                (number, message.text_message, message.phone_number, message.delivery_status, message.sent_time))
            number += 1
        except Exception as e:
            logging.error(e)
            continue
...
@is_user_customer
def experiential_payments(request):
    payments = ExperientialMpesa.objects.all()

    if request.method == 'POST':
        start_date = request.POST['start_date']
        end_date = request.POST['end_date']
        payments = ExperientialMpesa.objects.filter(created_at__gte=start_date, created_at__lte=end_date)
        context = {
            'payments': payments,
        ...
        return render(request, 'payments/experiential_payments.html', context)
    context = {
        'payments': payments,
        'total': payments.aggregate(Sum('amount'))['amount__sum'],
        'transaction_count': payments.count(),
...
def lipa_na_mpesa_online(request):
    if request.method == "POST":
        ...
        business_short_code = "7690604"
        passkey = Roberms.pass_key
        ...
        if phone_number_validates:
            ...
            amount = request.POST['amount']
            api_url = "https://api.safaricom.co.ke/mpesa/stkpush/v2/processrequest"
            ...
            request_mpesa = {
                ...
                "CallBackURL": f"https://roberms.co.ke/integrations/roberms/stk-push/confirmation/{actual_phone_number}/",
            ...
            messages.success(request,
                             'We have sent the Mpesa payment details to your phone number. Proceed to make payment by putting your Mpesa pin')
            return redirect('sms:verify_payment')
        else:
            messages.error(request, 'Invalid phone number format, make sure phone number start with 254')
            return redirect('sms:customer_credit')
    else:
        return redirect('sms:customer_credit')


# def clean_home_land_data(request):
#     messages = OutgoingDone.objects.raw(
#         "SELECT DISTINCT  `id`, `phone_number` FROM `sms_outgoingdone` WHERE `access_code` = 'HOMELAND' "
#         "and (`delivery_status` = 'SenderName Blacklisted' or `delivery_status`= 'AbsentSubscriber' or `delivery_status`= 'DeliveryImpossible')"
#     )
#     phone_numbers = []
#     for message in messages:
#         phone_numbers.append(message.phone_number)
#
#     contacts = Contact.objects.filter(group_id=1075, phone_number__in=list(set(phone_numbers)))
#     contacts.delete()


# contacts = OutgoingDone.objects.raw("SELECT DISTINCT `id`, `text_message`, `phone_number` FROM `sms_outgoingdone` WHERE `access_code` = 'HOMELAND'")
#     new_contacts = []
#     for contact in list(set(contacts)):
#         name = contact.text_message.split(',', 1)[0]
