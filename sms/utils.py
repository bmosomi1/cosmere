...
from roberms_admin.models import EmalifyToken
from sms.models import OutgoingDone


logging.basicConfig(filename="tokens_get.log", level=logging.INFO)
...
def get_access():
    ...
    if EmalifyToken.objects.count() > 0:
        em = EmalifyToken.objects.filter().first()
        em.token = json.loads(response.text)['access_token']
        em.save()
    else:
...
def get_access_token():
    if EmalifyToken.objects.count() > 0:
        ...
        seconds = (date_now - em.updated_at).total_seconds()

        if int(seconds) > 3000:
            token = get_access()
            logging.info(f'Token from emalify {token}')
            return token
        else:
            logging.info(f'Token from db {em.token}')
            return em.token
    else:
        token = get_access()
        logging.info(f'Token from db did not exist {token}')
        return token
